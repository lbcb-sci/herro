...
def estimate_error_bam(bam_path, restriction_path:str, length_threshold:int):
    ...
    with pysam.AlignmentFile(bam_path) as bam:
        for record in bam.fetch(until_eof=True): 
            ...
            if record.reference_name in SET_TO_EXCLUDE:
                num_excluded += 1
                continue
            read_len = record.infer_read_length()
            ...
            error_rate = (num_mismatch + short_ins + short_del) / read_len * 100
            mismatch_rate = (num_mismatch) / read_len * 100
            indel_rate = (short_ins + short_del) /read_len * 100 
    ...
    print('percentage mapped', (num_values + num_excluded) / (num_values + num_excluded + num_unmapped) * 100)
